namespace RealTime.Utils.UIUtils
{
    ...
    public interface IUIFastListRow
    {
        ...
        public event MouseEventHandler eventMouseEnter;
        #endregion
    }
    ...
    internal class UIFastList : UIComponent
    {
        ...
        private Color32 m_color = new(255, 255, 255, 255);
        ...
        public float listPosition
        {
            ...
            set
            {
                ...
                if (m_pos != value)
                {
                    float pos = Mathf.Max(Mathf.Min(value, m_rowsData.m_size - height / rowHeight), 0);
        ...
        public int selectedIndex
        {
            ...
            set
            {
                ...
                field = Mathf.Min(Mathf.Max(-1, value), m_rowsData.m_size - 1);

                int pos = Mathf.FloorToInt(m_pos);
                int newRowId = Mathf.Max(-1, field - pos);
                ...
                if (newRowId >= 0)
                {
                    ...
                    m_rows[m_selectedRowId].Select(field % 2 == 1);
                }

                if (eventSelectedIndexChanged != null && field != oldId)
                {
                    eventSelectedIndexChanged(this, field);
                }
            }
        } = -1;

        public object selectedItem => selectedIndex == -1 ? null : m_rowsData.m_buffer[selectedIndex];
        ...
        {
            get => m_stepSize > 0 ? m_stepSize : rowHeight; set => m_stepSize = value;
        }
        #endregion
        ...
        {
            if (m_rowsData == null || rowHeight <= 0)
            {
                return;
            }
            ...
            m_pos = Mathf.Max(Mathf.Min(pos, m_rowsData.m_size - height / rowHeight), 0f);
            ...
            for (int i = 0; i < m_rows.m_size; i++)
            {
                ...
                if (dataPos < m_rowsData.m_size)
                {
                    ...
                    if (dataPos == selectedIndex && m_updateContent)
                    {
                        m_selectedRowId = i;
        ...
        /// Refresh the display
        /// </summary>
        public void Refresh() => DisplayAt(m_pos);
        #endregion
        ...
        protected override void OnMouseWheel(UIMouseEventParameter p)
        {
            ...
            listPosition = m_stepSize > 0 && rowHeight > 0 ? m_pos - p.wheelDelta * m_stepSize / rowHeight : m_pos - p.wheelDelta;
        ...
        {
            if (m_panel == null || rowHeight <= 0)
            {
                return;
            }

            int nbRows = Mathf.CeilToInt(height / rowHeight) + 1;
            ...
            if (m_rows.m_size < nbRows)
            {
                ...
                for (int i = m_rows.m_size; i < nbRows; i++)
                {
                    ...
                    if (canSelect && !selectOnMouseEnter)
                    {
                        m_rows[i].eventClick += OnRowClicked;
                    }
                    else if (canSelect)
        ...
        private void UpdateScrollbar()
        {
            ...
            {
                bool isVisible = m_rowsData.m_size * rowHeight > height;
            ...
            _ = rowHeight * m_rowsData.m_size;
            float scrollSize = height * height / (rowHeight * m_rowsData.m_size);
            float amount = stepSize * height / (rowHeight * m_rowsData.m_size);
        ...
        private void UpdateScrollPosition()
        {
            ...
            float pos = m_pos * (height - m_scrollbar.scrollSize) / (m_rowsData.m_size - height / rowHeight);
        ...
        private void SetupControls()
        {
            ...
            m_panel.backgroundSprite = backgroundSprite;
            ...
            m_scrollbar.eventValueChanged += (c, t) =>
            {
                ...
                listPosition = m_scrollbar.value * (m_rowsData.m_size - height / rowHeight) / (height - m_scrollbar.scrollSize - 1f);
                m_lock = false;
            };
        }
        #endregion
